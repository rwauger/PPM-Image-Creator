...
/*
Purpose:
	The purpose of the function is to read in the info from the input file
Inputs:
	inf = the input file we are reading
Outputs:
	none
*/
void Triangle::readInfo(ifstream& inf){
...
bool Triangle::isHit(Point& test){
	...
	a = ((pts.at(1).gety()-pts.at(2).gety())*(test.getx()-pts.at(2).getx())+
		(pts.at(2).getx()-pts.at(1).getx())*(test.gety()-pts.at(2).gety()))/
		((pts.at(1).gety()-pts.at(2).gety())*
		(pts.at(0).getx()-pts.at(2).getx())+(pts.at(2).getx()-pts.at(1).getx())*
		(pts.at(0).gety()-pts.at(2).gety()));

	b = ((pts.at(2).gety()-pts.at(0).gety())*(test.getx()-pts.at(2).getx())+
		(pts.at(0).getx()-pts.at(2).getx())*
		(test.gety()-pts.at(2).gety()))/((pts.at(1).gety()-pts.at(2).gety())*
		(pts.at(0).getx()-pts.at(2).getx())+
		(pts.at(2).getx()-pts.at(1).getx())*(pts.at(0).gety()-pts.at(2).gety()));

	c = 1 - a - b;

	//check to see if the point falls inside the triangle
